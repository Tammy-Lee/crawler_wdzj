...
def make_response(api, with_login=False, with_proxy=False, timeout=0.5, **kwargs):
    """构造response对象
    ...
    response = requests.request(method, url=url, data=formdata, headers=headers, proxies=proxy, timeout=timeout)
    return response


def update_one(data, primary_keys, collection_name, upsert_type="$set", filter_key="_id"):
    _id = "_".join(data[key] for key in primary_keys)
    data["_id"] = _id
    rst = mongo_db[collection_name].update_one({filter_key: _id}, {upsert_type: data}, upsert=True)
    return rst
